...
const SCREENS: Screen[] = [
  { type: "text", lines: [
    { text: "Toda escolha tem um custo.", style: "serif-lead" },
    { text: "", style: "spacer" },
    { text: "Mesmo quando você decide ficar.", style: "body" },
    { text: "Mesmo quando decide esperar.", style: "body" },
    { text: "Mesmo quando decide entender mais uma vez.", style: "body" },
    { text: "", style: "spacer" },
    { text: "Nem sempre você paga com dinheiro.", style: "body" },
    { text: "Às vezes paga com energia.", style: "body" },
    { text: "Com tranquilidade.", style: "body" },
    { text: "Com clareza.", style: "body" },
    { text: "", style: "spacer" },
    { text: "E esse custo raramente é imediato.", style: "body" },
    { text: "Ele se acumula.", style: "serif-close" },
  ], button: "Continuar" },
  { type: "text", lines: [
    { text: "Cada vez que você tolera algo que te incomoda,", style: "serif-lead" },
    { text: "algo dentro de você se adapta.", style: "body" },
    { text: "", style: "spacer" },
    { text: "Você se torna mais paciente.", style: "body" },
    { text: "Mais compreensiva.", style: "body" },
    { text: "Mais flexível.", style: "body" },
    { text: "", style: "spacer" },
    { text: "Mas também, às vezes,", style: "body" },
    { text: "mais silenciosa consigo mesma.", style: "body" },
    { text: "", style: "spacer" },
    { text: "E enquanto investe em sustentar algo incerto,", style: "highlight-start" },
    { text: "adia algo possível.", style: "highlight-end" },
  ], button: "Continuar" },
  { type: "text", lines: [
    { text: "Não se trata de julgar suas decisões passadas.", style: "serif-lead" },
    { text: "Se trata de olhar a troca.", style: "body" },
    { text: "", style: "spacer" },
    { text: "O que você recebia?", style: "body" },
    { text: "O que entregava?", style: "body" },
    { text: "Era equilíbrio?", style: "body" },
    { text: "", style: "spacer" },
    { text: "Ou era você quem ajustava sempre primeiro?", style: "highlight-start" },
    { text: "", style: "highlight-end" },
    { text: "", style: "spacer" },
    { text: "O amor não deveria parecer uma compensação constante.", style: "serif-close" },
  ], button: "Continuar" },
  { type: "text", lines: [
    { text: "Hoje não se trata de cortar nada.", style: "serif-lead" },
    { text: "Nem de prometer nada.", style: "body" },
    { text: "Apenas de reconhecer o custo.", style: "body" },
    { text: "", style: "spacer" },
    { text: "Pergunte-se com honestidade:", style: "body" },
    { text: "", style: "spacer" },
    { text: "O que eu deixei de lado para sustentar essa conexão?", style: "body" },
    { text: "", style: "spacer" },
    { text: "Reduzi meus padrões?", style: "body" },
    { text: "Minha paz?", style: "body" },
    { text: "Minha energia?", style: "body" },
    { text: "Meu tempo?", style: "body" },
    { text: "", style: "spacer" },
    { text: "Responda sem dramatizar.", style: "body" },
    { text: "Apenas com clareza.", style: "serif-close" },
  ], button: "Continuar" },
  { type: "journal", title: "Reflexão", prompt: "Se eu continuar exatamente na mesma dinâmica por mais um ano… o que estaria perdendo?", hint: "(Não imagine o pior. Apenas seja honesta consigo.)", button: "Salvar e continuar" },
  { type: "text", lines: [
    { text: "Reconhecer o custo é o primeiro passo para parar de pagá-lo.", style: "serif-lead" },
  ], button: "Completar Dia 6" },
];

const TOTAL_SCREENS = SCREENS.length;

const DayExperience6 = () => {
  ...
  useEffect(() => { const saved = localStorage.getItem(`astra_day6_journal_${user?.id}`); if (saved) setJournalText(saved); }, [user?.id]);
  useEffect(() => { if (journalText && user?.id) localStorage.setItem(`astra_day6_journal_${user.id}`, journalText); }, [journalText, user?.id]);
  ...
  const handleComplete = async () => { if (!user) return; try { const now = new Date().toISOString(); const { data: current } = await supabase.from("audit_progress").select("current_day").eq("user_id", user.id).maybeSingle(); const updateData: Record<string, any> = { day_6_completed_at: now }; if (!current || current.current_day < 7) updateData.current_day = 7; await supabase.from("audit_progress").update(updateData).eq("user_id", user.id); queryClient.invalidateQueries({ queryKey: ["audit_progress", user.id] }); } catch (e) { console.error("Error completing day:", e); } setShowCompletion(true); };
  const screen = SCREENS[currentScreen];

  if (isLoading) {
    return <DayLoadingScreen dayNumber={6} dayTitle={DAY_TITLES[6]} onComplete={handleLoadingComplete} />;
  }
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center min-h-[80dvh] px-8">
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }} className="flex flex-col items-center text-center relative z-10">
          ...
          <h2 className="text-xl text-foreground mb-2" style={{ fontWeight: 200, letterSpacing: "-0.02em", lineHeight: 1.1 }}>Dia 6 completado</h2>
      ...
      <div className="px-5 pt-3 pb-2 relative z-10 shrink-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-center">
            <p className="text-[10px] font-medium tracking-[0.15em] uppercase text-primary/70">Dia 6</p>
            <p className="text-sm text-foreground/70" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300, letterSpacing: "-0.02em", lineHeight: 1.1 }}>O Custo da Oportunidade</p>
          </div>
