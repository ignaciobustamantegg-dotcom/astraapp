...
const DayExperience = () => {
  ...
  const { journalText, setJournalText, saveJournal } = useJournal(user?.id, 1);
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const updateData: Record<string, any> = { day_1_completed_at: now };
      if (!current || current.current_day < 2) {
  ...
  if (isLoading) {
    return <DayLoadingScreen dayNumber={1} dayTitle={DAY_TITLES[1]} onComplete={handleLoadingComplete} />;
  }
