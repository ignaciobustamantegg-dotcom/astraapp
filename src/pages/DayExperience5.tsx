...
const DayExperience5 = () => {
  ...
  const handleComplete = async () => { if (!user) return; try { const now = new Date().toISOString(); const { data: current } = await supabase.from("audit_progress").select("current_day").eq("user_id", user.id).maybeSingle(); const updateData: Record<string, any> = { day_5_completed_at: now }; if (!current || current.current_day < 6) updateData.current_day = 6; await supabase.from("audit_progress").update(updateData).eq("user_id", user.id); queryClient.invalidateQueries({ queryKey: ["audit_progress", user.id] }); } catch (e) { console.error("Error completing day:", e); } setShowCompletion(true); };
